...
{
    const SHOPPER_ECI = '0103910';
    const SHOPPER_PSERV = '17913101';
    const SHOPPER_VERSION_DF = '2';
    ...
    public function init()
    {
        ...
        add_action('woocommerce_checkout_process', [$this, 'validate_checkout_identification_field']);
        add_action('woocommerce_checkout_create_order', [$this, 'save_checkout_identification_field'], 10, 2);
        add_action('woocommerce_admin_order_data_after_billing_address', [$this, 'render_admin_identification_field']);
    ...
    public function validate_checkout_identification_field()
    {
        $paymentMethod = sanitize_text_field(wp_unslash($_POST['payment_method'] ?? ''));
        if ($paymentMethod !== 'uix_datafast') {
            return;
        }

        $rawValue = sanitize_text_field(wp_unslash($_POST['billing_identification_doc_id'] ?? ''));
        UIX_DF_Rec_Logger::info('Checkout field leído', [
            'payment_method' => $paymentMethod,
            'billing_identification_doc_id' => self::mask_document($rawValue),
        ]);

        if ($rawValue === '') {
            UIX_DF_Rec_Logger::info('Errores de validación checkout', ['field' => 'billing_identification_doc_id', 'reason' => 'empty']);
            wc_add_notice('El campo "Cédula o RUC" es obligatorio para pagar con Datafast.', 'error');
            return;
        }

        $normalized = self::normalize_identification($rawValue);
        if ($normalized === '') {
            UIX_DF_Rec_Logger::info('Errores de validación checkout', ['field' => 'billing_identification_doc_id', 'reason' => 'invalid_digits']);
            wc_add_notice('La cédula o RUC debe contener solo números válidos para Datafast.', 'error');
            return;
        }

        UIX_DF_Rec_Logger::info('Valor normalizado identificación', [
            'original' => self::mask_document($rawValue),
            'normalized' => self::mask_document($normalized),
        ]);
    }
    ...
    public function save_checkout_identification_field($order, $data)
    {
        ...
        $normalized = self::normalize_identification($rawValue);
        $order->update_meta_data('_billing_identification_doc_id', $rawValue);
        $order->update_meta_data('_uix_df_identification_doc_id_normalized', $normalized);
        UIX_DF_Rec_Logger::info('Checkout field guardado en order meta', [
            'order_id' => $order->get_id(),
            'raw' => self::mask_document($rawValue),
            'normalized' => self::mask_document($normalized),
        ]);
    }

    public function render_admin_identification_field($order)
    {
        $rawValue = (string) $order->get_meta('_billing_identification_doc_id', true);
        if ($rawValue === '') {
            return;
        }

        echo '<p><strong>Cédula o RUC:</strong> ' . esc_html($rawValue) . '</p>';
    }
    ...
    public function handle_create_checkout()
    {
        ...
        $payload = [
            ...
            'customParameters[SHOPPER_VERSIONDF]' => self::SHOPPER_VERSION_DF,
            ...
            'customParameters[SHOPPER_ECI]' => self::SHOPPER_ECI,
            'customParameters[SHOPPER_PSERV]' => self::SHOPPER_PSERV,
    ...
    {
        $raw = (string) $order->get_meta('_billing_identification_doc_id', true);
        if ($raw !== '') {
            return $raw;
        }

        $legacy = (string) $order->get_meta('_billing_cedula', true);
        return $legacy;
    }

    public static function mask_document($value)
    {
